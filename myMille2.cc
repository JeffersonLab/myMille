...
int main()
{
  ...
  Mille myMille( "outfile2.bin",b1,b2); // pass in initial values when variable is created

  // read data from a file in csv format into an array that is row(nevents), measurement(nmeas), data(index, xi, zi)
  // each number has to be followed by a comma even the last number in the row (csv format).
  int nevents = 20, irow=0, ncol=24, ndim=3, nmeas=8, idim, imeas;
  float data[nevents][nmeas][ndim];
  ifstream inputFile("manyEvents.dat");

  // loop over the nevents events in the file.
  for(irow=0; irow < nevents; irow++) {

    // unpack the numbers in each line.
    std::string line;
	...
	stringstream(val) >> data[irow][imeas][idim];
	//cout << "test=" << data[irow][imeas][idim] << " row=" << irow+1 << " meas=" << imeas+1 << " component=" << idim+1 << endl;
      }
